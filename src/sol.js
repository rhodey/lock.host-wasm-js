...
const addressFromStr = async (str) => {
  return address(str)
}
...
const addressFromSeed = async (seed) => {
...
const signerFromSeed = async (seed) => {
  const priv = textSeed32(seed)
  const pub = ed.getPublicKey(priv)
  const senderAddressStr = bs58.encode(pub)
...
const getBalance = async (address, rpc) => {
  ...
  const balance = Number(lamports) / 1_000_000_000
  return balance
}
...
export { addressFromStr, addressFromSeed, signerFromSeed, getBalance, transfer }
